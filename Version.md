...
## v1.17.6
**Fix — Hata Bildir özelliğiyle GitHub'a gelen açık kullanıcı raporlarının otomatik düzeltme turu (10 açık `durum:yeni` issue, eşik aşıldı — issue #15,#16,#17,#18,#19,#20,#21)**
- **#15 — Kod bloğu başlığı sağa dayalı duruyordu + aşağı scroll'da kayıyordu:** `.cb-toolbar` eskiden `position:absolute` overlay idi, DOM'da `<pre>`'in en sonuna ekleniyordu — daraltılmış (`overflow-y:auto`) bloklarda içerik aşağı kaydırılınca header de birlikte kayıp gidiyordu, tüm bar sağa yaslı olduğu için başlık de "sağa dayalı" görünüyordu. Artık `pre`'nin İLK çocuğu + `position:sticky top:0` (dikey scroll'da üstte sabit kalıyor), `justify-content:space-between` ile başlık sola/düğme grubu sağa yaslanıyor. `#content pre`/`.fp-editor-content pre`'nin üst padding'i (eskiden overlay'e yer açan 30px/28px) artık normal akışa göre küçültüldü; `.cb-toolbar`'ın negatif margin'i pre'nin kendi padding'iyle eşleşecek şekilde iki yüzey için ayrı ayarlandı (bkz. `Comments.json` → `why-cb-toolbar-must-be-first-child-with-matched-negative-margin`)
- **#16 — Kod bloğu içeren not açıldığında blok daraltılmış gelmiyordu:** `inflateCodeBlocks()` eskiden yalnızca >8 satırlık blokları otomatik daraltıyordu; artık yeni bir `forceCollapse` parametresi var — yalnızca "not açılışı" çağrı noktalarından (`editNote()`, float panel `loadNote()`) geçiriliyor ve uzunluğa/`data-cb-touched`'a bakmaksızın TÜM blokları daraltıyor. Aktif düzenleme sırasındaki diğer çağrılar (undo/redo, şablon uygulama, yeni blok ekleme) bu parametreyi geçirmiyor — aksi halde kullanıcı az önce genişlettiği bir blok her re-render'da geri kapanırdı
- **#17 — Satır bazlı bookmark, tıklanan satırı değil tüm bloğu (özellikle AI cevaplarını) işaretliyordu:** Gutter tıklama + toolbar bookmark eylemi (`js/06`, `js/08` — iki ayrı, birbirini yansıtan implementasyon) DOM'da yukarı tırmanırken yalnızca `todo-item` sınıflı `<li>`'de veya `#content`'in doğrudan çocuğunda duruyordu; normal madde imi/numaralı liste `<li>`'leri ile `.ai-block` içindeki paragraf/başlık/liste gibi çok satırlı sarmalayıcıların İÇİNDEKİ satırlar bu şartları hiç karşılamıyordu, tırmanma en yakın üst sarmalayıcıya (tüm `<ul>` veya tüm `.ai-block`) kadar çıkıyordu — bookmark ikonu (`top:50%`) o zaman bloğun dikey ortasına düşüyordu. Artık her `<li>` (todo dahil) hemen duruyor, `.ai-block`'un doğrudan çocuğu da `.col-panel-content`/`.layout-col` ile aynı "dur" listesine eklendi. Eşleşen CSS de eklendi: `li[data-bookmark]:not(.todo-item)`, `.ai-block > [data-bookmark]` (bkz. `Comments.json` → `trap-bookmark-walk-must-stop-at-any-li-and-ai-block-child`)
- **#18 — Dış bağlantı önizleme paneli açılana kadar (3sn bekleme + erişilebilirlik fetch'i) kullanıcı bir şey olup olmadığını anlamıyordu:** Hover başlar başlamaz linkin yanında dönen bir spinner ikonu gösteriliyor (`.ext-link-spinner`, mevcut `ai-tb-spin` keyframe'i yeniden kullanılıyor); panel açılınca, erişilebilirlik kontrolü olumsuz sonuçlanınca veya kullanıcı mouseout yapınca kaldırılıyor
- **#19 — Bookmark'lar arasında gezinme yolu yoktu:** Bookmark gutter'ı (sol oluk, x<20) üzerine gelince imlecin yanında küçük bir panel açılıyor — üstte önceki, altta sonraki bookmark'a `scrollIntoView({behavior:'smooth'})` ile atlayan iki ikon düğme. Notta hiç bookmark yoksa panel hiç gösterilmiyor. `bm.prev`/`bm.next` anahtarları 5 dile eklendi
- **#20 — Mobilde Panel bloklarının yüksekliği satır sayısına uymuyordu, son satır kenara yapışıyordu:** `.noted-grid.grid-panel tbody td { height:52px }` masaüstünde `table-cell` bağlamında minimum gibi davranır (içerik büyüdükçe hücre büyür) ama mobil media query'de `display:block`'a geçilince aynı `height` KESİN bir kutuya dönüşüyordu — çok satırlı içerik taşıyor, son satır kartın alt kenarına yapışıyordu. Mobil kuralda `height:auto` eklendi
- **#21 — Mobilde editör hamburger menüsü Not Başlığı'nın ilk harfinin üstüne biniyordu:** Masaüstünde `.editor-top`'un 30px sol padding'i `#editor-menu-wrap`'e (position:absolute, left:4px) yer açıyordu; mobil media query bu padding'i 12px'e düşürüyordu ama düğme için başka yer ayırmıyordu. Kullanıcı isteği doğrultusunda düğme sağa taşındı (`right:4px`), padding sağa kaydırıldı, dropdown da (`#editor-menu-dropdown`) taşmaması için sola açılacak şekilde ters çevrildi
- **Atlanan (kod değiştirilmedi):** **#7** — aynı kod bloğu başlık/wrap özelliği zaten v1.17.5'te uygulanmıştı (issue açık kalmış ama kod tabanında karşılığı mevcut, tekrar eden). **#22** ("CCB'lerin dokunmatik özelliği yok sanki") — kök neden statik incelemeyle netleştirilemedi (CCB `iframe`'i `sandbox="allow-scripts allow-forms allow-modals"` ile zaten touch'ı engelleyen bir CSS/JS bulunamadı; olası aday mobile Safari'nin `contenteditable=false` ada elementlerindeki bilinen touch-selection tuzağı ama bu yalnızca gerçek dokunmatik cihazda doğrulanabilir) — tahmine dayalı bir değişiklik yapmak yerine issue açık bırakıldı, elle/canlı cihazda incelenmesi öneriliyor
- **Doğrulama notu:** Bu tur otomatik/gözetimsiz çalıştı, tarayıcı/canlı cihaz erişimi yoktu — değişiklikler statik kod incelemesi + `node tools/comments-check.js` ile doğrulandı, gerçek dokunma/hover/scroll akışı ile CANLI doğrulanmadı
